...
test('督導檢視顯示所選案場整月全員班表，排班編輯維持不變',async()=>{
  ...
  assert.match(app,/function renderSupervisorScheduleHalf/);
  ...
  assert.match(html,/assets\/app\.js\?v=117/);
  assert.match(html,/assets\/app\.css\?v=67/);
});
...
test('正式報價單提供大型用印區並抑制瀏覽器列印網址頁尾',async()=>{
  ...
  assert.match(html,/assets\/tender-quotes\.js\?v=12/);
});
