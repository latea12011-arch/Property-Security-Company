...
test('ERP 管理端具備手機完整選單、登出、卡片表格與全螢幕表單',async()=>{const[html,js,css]=await Promise.all([read('index.html'),read('assets/app.js'),read('assets/app.css')]);assert.match(html,/id="mobileMoreButton"/);assert.match(html,/id="mobileMoreBackdrop"/);assert.match(html,/id="mobileLogoutButton"/);assert.match(js,/mobileMenuSections/);assert.match(js,/mobileLogoutButton/);assert.match(js,/prepareMobileTables/);assert.match(css,/mobile-card-table/);assert.match(css,/height:100dvh/)});
