...
const demoKey='hongjia_tender_contracts_demo',quoteDemoKey='hongjia_tender_quotes_demo',logoPath='assets/contracts/hongjia-property-mark.png';
...
async function syncQuoteToForm(quoteId,forceType){const quote=quotes.find(x=>x.id===quoteId),detail=await quoteItems(quoteId);if(!quote)return;const f=$('#tenderContractForm'),type=forceType||f.contract_type.value||chooseType(detail);f.contract_type.value=type;f.company_name.value=companyFor(type).name;f.client_name.value=quote.client_name||'';f.project_name.value=quote.project_name||'';f.site_address.value=quote.site_address||'';f.contract_start_date.value=quote.contract_start_date||'';f.contract_end_date.value=quote.contract_end_date||'';f.contract_months.value=quote.contract_months||12;f.tax_rate.value=quote.tax_rate??5;if(!f.service_scope.value)f.service_scope.value=defaultScope(type);renderItems(fromQuoteItems(detail,type))}
...
async function buildContractDocument(row,items){const d=mergeDetails(row),company=companyFor(row.contract_type),logo=await loadLogo(),typeName=types[row.contract_type]||types.property,start=row.contract_start_date||'____年____月____日',end=row.contract_end_date||'____年____月____日',monthly=money(row.monthly_total),terms=row.contract_terms||defaultTerms(),equipment=checkedList(equipmentOptions,d.equipment),expenses=checkedList(expenseOptions,d.expenses),staffRows=items.map(x=>`<tr><td>${esc(x.role_name)}</td><td>${esc(x.headcount)}</td><td>${esc(x.work_description||'')}</td><td>${money(x.unit_monthly_amount)}</td><td>${money(x.line_monthly_total||Number(x.headcount)*Number(x.unit_monthly_amount))}</td></tr>`).join('');
  const articles=[
    ...
    article('六','服務費用及付款方式',`<p>每月含稅服務費新臺幣 <b>${monthly}</b> 元整。甲方應於次月 ${esc(row.payment_due_day||10)} 日前，以匯款或雙方指定方式支付。</p><p>銀行：${esc(d.bank_name||'________________')}　分行：${esc(d.bank_branch||'________________')}</p><p>戶名：${esc(d.bank_account_name||company.name)}　帳號：${esc(d.bank_account_no||'________________')}</p><p>如遇基本工資、法令或必要人事成本重大調整，雙方得以書面協議調整服務費。甲方逾期未繳，經催告仍未於十日內繳付者，乙方得依法終止契約。</p>`),
  ...
  <div class="page"><h1 class="doc-title">立契約書者</h1><div class="sign-grid"><div class="sign-box"><b>甲方：${esc(row.client_name||'________________')}</b><p>代表人：${esc(row.client_representative||'________________')}　職稱：${esc(d.client_title||'________________')}</p><p>簽章：</p><p>地址：${esc(row.site_address||'________________')}</p><p>電話：${esc(d.client_phone||'________________')}</p><p>統一編號：${esc(d.client_tax_id||'________________')}</p></div><div class="sign-box"><b>乙方：${esc(company.name)}</b><p>負責人：${esc(d.company_representative||'________________')}</p><p>簽章：</p><p>地址：${esc(company.address)}</p><p>電話：${esc(company.phone)}</p><p>統一編號：${esc(company.taxId)}</p></div></div><p style="text-align:center;margin-top:35mm">${esc(rocDate(d.signing_date))}</p></div>
