(() => {
  ...
  function renderTable() {
    ...
    tbody.innerHTML = rows.length ? rows.map(row => `
      <tr>
        ...
        <td><button class="mini-button" type="button" data-police-edit="${esc(row.employee_id)}">編輯核備</button></td>
      </tr>
    `).join('') : '<tr><td colspan="8" class="empty">沒有符合條件的員工。</td></tr>';
    ...
    dialog.innerHTML = `
      <form id="policeApprovalForm">
        ...
        <div class="form-grid">
          ...
          <label>警局收文／核備文號<input name="document_no"></label>
  ...
  async function save(event) {
    ...
    const payload = {
      ...
      document_no: values.document_no || null,
  ...
  function printList() {
    ...
    frame.srcdoc = `<!doctype html><html lang="zh-TW"><head><meta charset="utf-8"><title>警局核備清冊</title><style>@page{size:A4 landscape;margin:10mm}body{font-family:"Microsoft JhengHei",sans-serif;color:#16324f}h1{text-align:center}p{text-align:right}table{width:100%;border-collapse:collapse}th,td{border:1px solid #8091a0;padding:8px;font-size:12px}th{background:#eaf0f4}</style></head><body><h1>紘嘉物業保全－警局核備清冊</h1><p>列印日期：${new Date().toLocaleDateString('zh-TW')}</p><table><thead><tr><th>工號</th><th>姓名</th><th>職稱</th><th>狀態</th><th>送件警局</th><th>送件日期</th><th>文號</th><th>核備日期</th></tr></thead><tbody>${rows.map(row => `<tr><td>${esc(row.employee_no)}</td><td>${esc(row.full_name)}</td><td>${esc(row.job_title)}</td><td>${esc(statusLabels[row.status])}</td><td>${esc(row.police_station)}</td><td>${esc(row.submitted_date)}</td><td>${esc(row.document_no)}</td><td>${esc(row.approval_date)}</td></tr>`).join('')}</tbody></table></body></html>`;
    frame.onload = () => {
  ...
  async function render() {
    ...
    try {
      ...
      content.innerHTML = `
        ...
        <article class="panel">
          <div class="panel-head police-approval-head">
            ...
            <button class="btn ghost" id="printPoliceApprovalList" type="button">下載／列印核備清冊</button>
          </div>
          ...
          <div class="table-wrap"><table><thead><tr><th>工號</th><th>員工</th><th>核備狀態</th><th>送件警局</th><th>送件日期</th><th>文號</th><th>核備日期</th><th>操作</th></tr></thead><tbody id="policeApprovalRows"></tbody></table></div>
        </article>`;
      ...
      $('#printPoliceApprovalList').onclick = printList;
      renderTable();
    } catch (error) {
