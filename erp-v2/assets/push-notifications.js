...
function render(rows){if(!target)return;target.innerHTML=rows.length?rows.map(item=>`<button type="button" class="notification-inbox-item ${item.read_at?'':'unread'}" data-notification-id="${item.id}"><strong>${esc(item.title)}</strong><span>${esc(item.body)}</span><small>${new Date(item.created_at).toLocaleString('zh-TW')}</small></button>`).join(''):'<div class="empty">目前沒有通知</div>';target.querySelectorAll('[data-notification-id]').forEach(button=>button.onclick=async()=>{const item=rows.find(row=>row.id===button.dataset.notificationId);await client.from('app_notifications').update({read_at:new Date().toISOString()}).eq('id',item.id);if(item.target_url)location.href=item.target_url;else load()})}
...
async function load(){if(!client||!target)return[];const{data,error}=await client.from('app_notifications').select('*').order('created_at',{ascending:false}).limit(30);if(error){target.innerHTML='<div class="empty">通知功能正在更新中</div>';return[]}render(data||[]);document.dispatchEvent(new CustomEvent('hongjia-notification-count',{detail:{count:(data||[]).filter(x=>!x.read_at).length}}));return data||[]}
async function dispatch(){try{await client.functions.invoke('push-dispatch',{body:{action:'dispatch'}})}catch(_){}
}
