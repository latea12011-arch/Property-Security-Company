...
function renderEmployee(rows){if(!rows.length){target.innerHTML='<div class="empty">目前沒有通知</div>';return}const recent=rows.slice(0,10),expanded=target.dataset.expanded==='true',visible=expanded?recent:recent.slice(0,3),unread=recent.filter(item=>!item.read_at).length,remaining=Math.max(0,recent.length-3);target.innerHTML=`<div class="employee-notification-summary"><span>最近通知</span><small>${unread?`${unread} 則未讀 · `:''}最多保留 10 則</small></div>${visible.map(notificationItem).join('')}${remaining?`<button type="button" class="employee-notification-toggle" data-notification-toggle>${expanded?'收合通知':`展開其餘通知（${remaining}）`}</button>`:''}`;bindNotificationItems(visible);target.querySelector('[data-notification-toggle]')?.addEventListener('click',()=>{target.dataset.expanded=expanded?'false':'true';renderEmployee(recent)})}
