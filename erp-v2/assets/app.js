(() => {
  ...
  function downloadSiteMonthlySchedule(rows,site){if(!rows.length)return showNotice('此案場本月尚無可下載的執勤班表。','error');const csv=v=>`"${String(v??'').replace(/"/g,'""')}"`,range=monthRange(state.scheduleMonth),byEmployee=new Map();for(const row of rows){if(!byEmployee.has(row.employee_id))byEmployee.set(row.employee_id,new Map());byEmployee.get(row.employee_id).set(row.work_date,row)}const shiftCell=row=>{if(!row)return'';const time=row.work_time_text||`${String(row.start_time||'').slice(0,5).replace(':00','')}-${String(row.end_time||'').slice(0,5).replace(':00','')}`,prefix=row.shift_type==='day'?'日 ':row.shift_type==='night'?'夜 ':'';return prefix?`${prefix}${time}`:`\u200B${time}`},dates=Array.from({length:range.days},(_,i)=>`${state.scheduleMonth}-${String(i+1).padStart(2,'0')}`),employees=[...byEmployee.keys()].map(id=>state.relations.employees.find(x=>x.id===id)||{id,employee_no:'',full_name:'未指定'}).sort((a,b)=>String(a.employee_no||'').localeCompare(String(b.employee_no||''))),lines=[['紘嘉物業保全股份有限公司－案場整月班表'],['案場',site?.name||''],['月份',`${range.y} 年 ${range.m} 月`],[],['工號','姓名',...dates.map((date,i)=>`${range.m}月${i+1}日`)],...employees.map(employee=>[employee.employee_no||'',employee.full_name||'',...dates.map(date=>shiftCell(byEmployee.get(employee.id)?.get(date)))])],content='\ufeff'+lines.map(line=>line.map(csv).join(',')).join('\r\n'),blob=new Blob([content],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a'),safe=String(site?.name||'案場').replace(/[\\/:*?"<>|]/g,'_');a.href=url;a.download=`${state.scheduleMonth}_${safe}_橫式整月班表.csv`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);showNotice(`已下載 ${employees.length} 位執勤人員的橫式月班表，可使用 Excel 開啟。`,'success');}
  ...
  async function renderCurrent() { try { state.view==='dashboard'?await renderDashboard():state.view==='schedules'?await renderEmployeeMonthlySchedule():state.view==='attendance'?await renderAttendanceRecords():state.view==='tenderQuotations'?await window.TenderQuotes.render():state.view==='websiteManager'?await window.WebsiteCenter.renderManager():state.view==='websiteNotifications'?await window.WebsiteCenter.renderNotifications():await renderTable(state.view); } catch(error) { $('#content').innerHTML=`<article class="panel empty">載入失敗：${esc(error.message)}</article>`; } }
