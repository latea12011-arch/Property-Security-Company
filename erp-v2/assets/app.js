(() => {
  ...
  function downloadSiteMonthlySchedule(rows,site){if(!rows.length)return showNotice('此案場本月尚無可下載的執勤班表。','error');const csv=v=>`"${String(v??'').replace(/"/g,'""')}"`,weekdays=['日','一','二','三','四','五','六'],shiftLabels={day:'日班',night:'夜班',mobile:'機動班',special:'特勤班',cash:'現金班（當日領現）',custom:'自訂班'},lines=[['紘嘉物業保全股份有限公司－案場整月執勤人員班表'],['案場',site?.name||''],['月份',state.scheduleMonth],['下載時間',new Date().toLocaleString('zh-TW')],[],['日期','星期','員工編號','執勤人員','班別','開始時間','結束時間','值勤時段','現金班金額','領現狀態'],...[...rows].sort((a,b)=>String(a.work_date).localeCompare(String(b.work_date))||String(format('employee_id',a.employee_id)).localeCompare(String(format('employee_id',b.employee_id)))).map(row=>{const employee=state.relations.employees.find(x=>x.id===row.employee_id),day=new Date(`${row.work_date}T00:00:00`).getDay();return[row.work_date,`週${weekdays[day]}`,employee?.employee_no||'',employee?.full_name||'',shiftLabels[row.shift_type]||row.shift_type,String(row.start_time||'').slice(0,5),String(row.end_time||'').slice(0,5),row.work_time_text||'',row.shift_type==='cash'?row.cash_amount||0:'',row.shift_type==='cash'?(row.cash_payment_status==='paid'?'已領現':'待領現'):''];})],content='\ufeff'+lines.map(line=>line.map(csv).join(',')).join('\r\n'),blob=new Blob([content],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a'),safe=String(site?.name||'案場').replace(/[\\/:*?"<>|]/g,'_');a.href=url;a.download=`${state.scheduleMonth}_${safe}_整月執勤人員班表.csv`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);showNotice(`已下載 ${rows.length} 筆案場執勤班表，可使用 Excel 開啟。`,'success');}
