(() => {
  ...
  let state = { view: 'dashboard', user: null, editing: null, relations: {employees:[],sites:[]}, scheduleMonth: new Date().toISOString().slice(0,7), scheduleSite: '' };
  ...
  async function renderCurrent() { try { state.view==='dashboard'?await renderDashboard():state.view==='schedules'?await renderMonthlySchedule():await renderTable(state.view); } catch(error) { $('#content').innerHTML=`<article class="panel empty">載入失敗：${esc(error.message)}</article>`; } }
