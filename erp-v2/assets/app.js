(() => {
  ...
  function downloadSiteMonthlySchedule(rows,site){if(!rows.length)return showNotice('此案場本月尚無可下載的執勤班表。','error');const csv=v=>`"${String(v??'').replace(/"/g,'""')}"`,weekdays=['日','一','二','三','四','五','六'],shiftLabels={mobile:'機動班',special:'特勤班',cash:'現金班',custom:'自訂班'},byDate=new Map();for(const row of rows){if(!byDate.has(row.work_date))byDate.set(row.work_date,[]);byDate.get(row.work_date).push(row)}const staff=row=>{const employee=state.relations.employees.find(x=>x.id===row.employee_id),name=employee?.full_name||employee?.employee_no||'未指定',time=row.work_time_text||`${String(row.start_time||'').slice(0,5)}-${String(row.end_time||'').slice(0,5)}`;return`${name}${time&&time!=='-'?`（${time}）`:''}`},lines=[['紘嘉物業保全股份有限公司－案場整月班表'],['案場',site?.name||''],['月份',state.scheduleMonth],[],['日期','星期','日班人員','夜班人員','其他執勤'],...[...byDate.entries()].sort(([a],[b])=>a.localeCompare(b)).map(([date,list])=>{const day=new Date(`${date}T00:00:00`).getDay(),dayStaff=list.filter(x=>x.shift_type==='day').map(staff).join('、'),nightStaff=list.filter(x=>x.shift_type==='night').map(staff).join('、'),other=list.filter(x=>!['day','night'].includes(x.shift_type)).map(x=>`${shiftLabels[x.shift_type]||x.shift_type}：${staff(x)}`).join('、');return[date,`週${weekdays[day]}`,dayStaff,nightStaff,other]})],content='\ufeff'+lines.map(line=>line.map(csv).join(',')).join('\r\n'),blob=new Blob([content],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a'),safe=String(site?.name||'案場').replace(/[\\/:*?"<>|]/g,'_');a.href=url;a.download=`${state.scheduleMonth}_${safe}_整月簡易班表.csv`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);showNotice(`已下載 ${byDate.size} 天的案場簡易班表，可使用 Excel 開啟。`,'success');}
