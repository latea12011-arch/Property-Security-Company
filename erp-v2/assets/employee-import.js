(() => {
  ...
  const HEADER_ALIASES = {
    ...
    birth_date: ['出生年月日', '出生日期'],
    highest_education: ['最高學歷', '學歷'],
    registered_address: ['戶籍地址', '戶籍地'],
    residential_address: ['現居地址', '現居地'],
    emergency_contact_name: ['緊急聯絡人'],
    emergency_contact_relationship: ['緊急聯絡人關係', '與員工關係', '關係'],
  ...
  function parseMatrix(matrix) {
    ...
    matrix.slice(rowIndex + 1).forEach((row, offset) => {
      ...
      const record = {
        ...
        birth_date: dateValue(get('birth_date'), '出生年月日', errors),
        highest_education: get('highest_education') || null,
