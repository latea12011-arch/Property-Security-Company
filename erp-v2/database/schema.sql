...
create table public.sites (
  ...
  community_tax_id text check (community_tax_id is null or community_tax_id ~ '^[0-9]{8}$'),
  community_phone text,
  acquisition_source text not null default 'direct' check (acquisition_source in ('direct', 'partner')),
  referrer_name text,
  schedule_print_footer text,
  schedule_print_notes text,
  contact_name text,
  contact_phone text,
  status public.record_status not null default 'active',
  created_at timestamptz not null default now(),
...
create table public.schedules (
  ...
  shift_type text not null check (shift_type in ('day','night','mobile','special','cash','off','annual','personal','sick','custom')),
  duty_post text check (duty_post is null or duty_post in ('main','control','lane','patrol','secondary','lobby','gate','parking','reception','mobile_support','other')),
